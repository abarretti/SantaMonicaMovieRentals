...
class InventoryDAO extends DAO
{
	...
	public function getCompanyNames($productionCompanyName)
	{
    	...
    	while($row = $result->fetch_assoc()) 
    		{
        	echo "<option value='".$row["productionCompanyName"]."'";
        	if($productionCompanyName == $row["productionCompanyName"]) 
        		{
        			echo 'selected';
        		}
        	echo ">".$row["productionCompanyName"]."</option>";
    		}
		} 
		$conn->close();
	}
	...
	public function getGenres($action, $children, $comedy, $documentary, $drama, $horror, $musicals, $romance, $scienceFiction, $thriller)
	{
    	...
    	while($row = $result->fetch_assoc()) 
    		{
        	echo "<input type='checkbox' name='".lcfirst(str_replace(' ','',$row["genreName"]))."' value='".lcfirst(str_replace(' ','',$row["genreName"]))."' ";
